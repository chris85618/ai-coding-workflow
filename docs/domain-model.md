# Domain Model — Unified Agentic Workflow System

> **Generated by**: Stage 5 (s2c-domain-model.md)
> **輸入**: UC-001..UC-011, ALG-001..ALG-005
...
| Context | 通用語言 | 涵蓋 UC |
|---------|---------|---------|
| Pipeline Orchestration | Phase, Stage, Gate, Transition, Recovery | UC-001, UC-003, UC-010 |
| Traceability Engine | ID, TraceLink, Chain, Orphan, SemanticCheck, CrossLink | UC-004, UC-005, UC-011 |
...
MicroValidator
  ...
  [INV]  6 步依序執行，不可跳過
  [INV]  MAX_AUTO_FIX = 3
  [POST] validate(): PASS（變更紀錄已寫入 ADR）∨ FAIL（HITL 已上報）
  [POST] triggerImpactAnalysis(): ImpactRecord 已持久化至對應 ADR
```
...
ImpactAnalyzer
  ...
  [INV]  MAJOR → 強制上報 HITL
  [POST] analyze(): ImpactRecord 已寫入對應 ADR ∧ (若 MAJOR 則 hitl_notified = true)
```
...
### CLS-013: WorkflowResumer（領域服務）

**描述**：Session 開始時從 workflow-state.md 載入斷點資訊、組裝恢復摘要、呈現 HITL 選項（繼續/其他/重置）。
**追溯**：UC-010 (models)

```
<<domain service>>
WorkflowResumer
  + loadState(statePath: string): WorkflowSnapshot
  + assembleSummary(snapshot: WorkflowSnapshot): RecoverySummary
  + presentHitlChoices(summary: RecoverySummary): HitlRecoveryChoice {CONTINUE, OTHER, RESET}
  + applyRecovery(choice: HitlRecoveryChoice, snapshot: WorkflowSnapshot): void
  [PRE]  loadState(): statePath 存在且可讀
  [PRE]  applyRecovery(): choice 已由 HITL 確認
  [INV]  已通過的閘門不可重置（Gate Status 單調）
  [INV]  恢復後 Pipeline Position 不可倒退
  [POST] loadState(): WorkflowSnapshot 包含 pipeline_position, gate_status, pending_escalations
  [POST] applyRecovery(): workflow-state.md 已更新 ∧ currentPosition 與 snapshot 一致或前進
```

### CLS-014: CrossLinkTracer（領域服務）

**描述**：修改任何 ID 時，除縱向主鏈外，追溯所有方向連結（justifies/constrains/mitigates/emitted-by/formalizes），驗證受影響的 ADR 仍然成立、NFR 仍被滿足。
**追溯**：UC-011 (models)

```
<<domain service>>
CrossLinkTracer
  + traceAllDirections(changedId: TraceableID): CrossLinkReport
  - collectHorizontalLinks(id: TraceableID): TraceLink[]
  - verifyAdrValidity(affectedAdrs: ADR[]): AdrValidityResult[]
  - verifyNfrSatisfaction(affectedNfrs: NFR[]): NfrCheckResult[]
  + flagViolations(report: CrossLinkReport): Violation[]
  [PRE]  traceAllDirections(): changedId 已存在於 Registry ∧ 縱向 blast_radius 已由 ImpactAnalyzer 計算
  [PRE]  verifyAdrValidity(): affectedAdrs 非空
  [INV]  所有方向連結皆已掃描（非僅縱向）
  [INV]  ADR 不再成立時必須標記 SUPERSEDED
  [INV]  NFR 被違反時 severity 升至 MAJOR
  [POST] traceAllDirections(): CrossLinkReport 已產出 ∧ 所有受影響 ID 的完整文件已讀取
  [POST] flagViolations(): Violation[] 已輸出 ∧ (若含 ADR_SUPERSEDED 或 NFR_VIOLATED 則 HITL 已通知)
```
...
### EVT-006: WorkflowResumed

**描述**：Session 開始時從斷點恢復後觸發，通知系統從正確位置繼續。
**追溯**：CLS-013 (emitted-by)

```
<<domain event>>
WorkflowResumed
  - resumedPosition: PhaseOrStage
  - recoveryChoice: HitlRecoveryChoice
  - pendingEscalations: int
  - timestamp: datetime
```

### EVT-007: CrossLinkViolationDetected

**描述**：全鏈影響追溯發現 ADR 不再成立或 NFR 被違反時觸發。
**追溯**：CLS-014 (emitted-by)

```
<<domain event>>
CrossLinkViolationDetected
  - violationType: ViolationType {ADR_SUPERSEDED, NFR_VIOLATED}
  - affectedId: TraceableID
  - description: string
  - severity: Severity
```
