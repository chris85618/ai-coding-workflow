"""Traceability System — TraceabilityValidator Algorithm class.
...
class TraceabilityValidator:
    ...
    def detect_orphans(cls, nodes: list[TraceabilityNode]) -> list[str]:
        ...
        for node in nodes:
            ...
            if needs_upstream and not node.upstream and not any(link in valid_link_types for link in node.links):
                orphans.append(node.id)

            if needs_downstream and not node.downstream and not any(link in valid_link_types for link in node.links):
                orphans.append(node.id)
