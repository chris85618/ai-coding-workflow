...
4. **關聯治理產出物**：變更紀錄（IMP-xxx）、技術債（DEBT-xxx）、風險（RISK-xxx）皆追溯至授權它的 ADR
...
ADR-{CAT}-xxx（決策記錄）
  │
  ├── 觸發 → IMP-xxx（影響分析紀錄，見 IMPACT-ANALYSIS.md）
  │          └── blast_radius, 受影響 ID, 閘門重驗證
  │
  ├── 產生 → DEBT-xxx（技術債項目，見 TECH-DEBT.md）
  │          └── RICE 分數, 四象限分類, 修復計畫
  │
  ├── 紀錄 → IMP-xxx（變更紀錄，見 CHANGE-MANAGEMENT.md）
  │          └── 變更類型, 根因分類, 左移動作
  │
  └── 緩解 → RISK-xxx（風險項目）
             └── 影響 × 機率, 緩解策略
...
1. **每個 IMP-xxx 必須追溯至一個 ADR**：變更紀錄的 `授權 ADR` 欄位不可為空
2. **每個 DEBT-xxx 可追溯至 ADR**：技術債的 `產生原因 ADR` 欄位記錄導致該債務的決策
3. **MAJOR 影響分析結果 → 產出新 ADR**：記錄人類的處理決策
4. **FIX 類型變更 → LESSON-xxx 追溯至 ADR**：根因左移的教訓連結至原始決策
