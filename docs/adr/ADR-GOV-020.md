...
- **瓶頸識別**:
  - 問題發生點: AGENTS.md 新增內容時（多次 session 累積）
  - 逃逸路徑: 無「可執行指令密度」衡量指標 → 未將 token 效率視為設計約束 → 參考內容與執行指令混合
  - 最早可偵測點: 每次向 AGENTS.md 新增內容時
  - 瓶頸位置: `AGENTS.md` 缺少「內容判定規則」（執行性 vs 參考性）
  - 介入類型: NEW_GUARD
  - 預期覆蓋: AGENTS.md 膨脹（非執行性內容 inline 至執行協議）
