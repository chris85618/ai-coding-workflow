"""Adapter Layer — StateMapper for WorkflowState ↔ Domain Model bridge.

Maps between the WorkflowState TypedDict and domain objects.
...
class StateMapper:
    ...
    def pipeline_to_state(pipeline: Pipeline) -> WorkflowState:
        """Convert a Pipeline domain object to a workflow state dict.
    ...
    def state_to_pipeline(state: WorkflowState) -> Pipeline:
        """Reconstruct a Pipeline domain object from workflow state.
    ...
    def stage_to_state(stage: Stage) -> WorkflowState:
        """Convert a Stage domain object to a partial workflow state."""
    ...
    def state_to_stage(state: WorkflowState) -> Stage | None:
        """Reconstruct a Stage from workflow state."""
