"""SonarCloud quality gateway adapter implementation."""
...
class SonarCloudAdapterMapper(QualityGateway):
    ...
    @classmethod
    def _coerce_value(cls, raw: Any) -> float | str | None:
        """Coerce a raw string value to float if numeric, else keep as str."""
        is_str = isinstance(raw, str)
        return cls._parse_float(raw) if is_str else raw
    ...
    @classmethod
    def _fetch_all_measures(cls, client: Any, project_key: str, keys: list[str]) -> list[dict[str, Any]]:
        measures: list[dict[str, Any]] = []
        for i in range(0, len(keys), 50):
            measures.extend(cls._fetch_chunk_measures(client, project_key, keys[i : i + 50]))
        return measures

    @classmethod
    def _map_metrics(cls, measures: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        return {
            cls.KEY_MAP.get(m["metric"], m["metric"]): {"global": cls._coerce_value(m.get("value"))} for m in measures
        }
    ...
    @classmethod
    def _add_best_value(cls, detail: dict[str, Any], measure: dict[str, Any] | None) -> None:
        if cls._has_best_value(measure):
            detail["bestValue"] = measure["bestValue"]  # type: ignore

    @classmethod
    def _build_detail(cls, m: dict[str, Any], mmap: dict[str, Any]) -> dict[str, Any]:
        key = m["key"]
        measure = mmap.get(key)
        detail = dict(m)
        detail["value"] = cls._coerce_value(measure.get("value")) if measure else None
        cls._add_best_value(detail, measure)
        return detail
    ...
    def get_metrics(self, project_key: str | None = None) -> dict[str, dict[str, Any]]:
        ...
        """
        key = self._select_key(project_key, self.config.project_key)
        self._validate_project_key(key)
        measures = self._fetch_chunk_measures(self.client, self._get_str(key), self.METRIC_KEYS)
        return self._map_metrics(measures)
    ...
    def get_issues(self, include_closed: bool = False) -> list[dict[str, Any]]:
        ...
        Raises:
            RuntimeError: If the SonarCloud API call fails.
        """
        self._validate_project_key(self.config.project_key)
        res = self._fetch_issues(self.client, self._get_str(self.config.project_key))
        issues = res.get("issues", []) if isinstance(res, dict) else list(res)
        filtered = list(filter(lambda i: i.get("status") not in self.CLOSED_STATUSES, issues))
    ...
    def get_all_available_metrics(self) -> list[dict[str, Any]]:
        ...
        Raises:
            RuntimeError: If the SonarCloud API call fails.
        """
        return self._parse_metrics(self._fetch_metrics(self.client))
    ...
    def get_detailed_component_measures(self, metric_keys: list[str]) -> list[dict[str, Any]]:
        ...
        """
        self._validate_project_key(self.config.project_key)
        key = self._get_str(self.config.project_key)
        return self._fetch_all_measures(self.client, key, metric_keys) if metric_keys else []
...
SonarCloudAdapter = SonarCloudAdapterMapper
CLOSED_STATUSES = SonarCloudAdapterMapper.CLOSED_STATUSES
KEY_MAP = SonarCloudAdapterMapper.KEY_MAP
METRIC_KEYS = SonarCloudAdapterMapper.METRIC_KEYS
_coerce_value = SonarCloudAdapterMapper._coerce_value
_select_key = SonarCloudAdapterMapper._select_key
_get_str = SonarCloudAdapterMapper._get_str
