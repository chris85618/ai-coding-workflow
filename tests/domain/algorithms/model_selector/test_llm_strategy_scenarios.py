...
class TestLlmStrategyScenarios:
    """BDD scenarios for LLM strategy selection."""

    @staticmethod
    @scenario("features/llm_strategy.feature", "Agent alpha uses reasoning model for critique")
    def test_alpha_critique() -> None:
        """SC-014: Agent alpha → reasoning model on CRITIQUE."""

    @staticmethod
    @scenario("features/llm_strategy.feature", "Agent beta uses editing model for resolve")
    def test_beta_resolve() -> None:
        """SC-014: Agent beta → editing model on RESOLVE."""

    @staticmethod
    @scenario("features/llm_strategy.feature", "Simple formatting uses cheapest model")
    def test_format_cheap() -> None:
        """SC-014: FORMAT task → cheapest model."""

    @staticmethod
    @scenario("features/llm_strategy.feature", "Fallback when provider is disabled")
    def test_fallback_disabled_provider() -> None:
        """SC-014: Disabled provider → fallback model."""

    @staticmethod
    @scenario("features/llm_strategy.feature", "All configured providers are listed")
