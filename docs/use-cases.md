# Use Cases Register — Unified Agentic Workflow System
...
**描述**：Session 開始時偵測到 workflow-state.md 存在且有進行中工作，呈現恢復摘要（位置/閘門/待辦/上報），使用者確認後從斷點繼續或執行其他指令。
...
**例外流程**：workflow-state.md 損壞 → 從 ADR-GATE 和 docs/ 重建

---

## UC-011: 全鏈影響追溯

**描述**：修改任何 ID 時，除縱向主鏈外，追溯所有方向的連結（justifies/constrains/mitigates/emitted-by/formalizes），讀取每個受影響 ID 的完整文件，驗證語意一致性，確認所有 ADR 仍然成立、所有 NFR 仍被滿足。
**追溯**：FR-022 (realizes)
