...
class TestIterationConvergenceScenarios:
    """BDD scenarios for iteration convergence."""

    @staticmethod
    @scenario("features/iteration_convergence.feature", "Auto-convergence at fixed point (REACHED)")
    def test_fixed_point_reached() -> None:
        """SC-003: All YAGNI → REACHED → auto-pass."""

    @staticmethod
    @scenario("features/iteration_convergence.feature", "Max iterations reached auto-advances")
    def test_max_iterations() -> None:
        """SC-003: MAX_ITERATIONS → auto-advance with warning."""

    @staticmethod
    @scenario("features/iteration_convergence.feature", "NOT_REACHED continues autonomously")
    def test_not_reached_continues() -> None:
        """SC-003: CRITICAL findings → NOT_REACHED → continue."""

    @staticmethod
    @scenario("features/iteration_convergence.feature", "MAJOR impact logged but execution continues")
    def test_major_impact_continues() -> None:
        """SC-003: MAJOR impact → log + continue autonomously."""

    @staticmethod
    @scenario("features/iteration_convergence.feature", "Stage status transitions are unidirectional")
    def test_status_unidirectional() -> None:
        """SC-003: PENDING → ITERATING → PASSED is unidirectional (INV-003)."""

    @staticmethod
    @scenario("features/iteration_convergence.feature", "Micro-validation failure auto-retries then skips")
