"""Orchestration Adapter — Workflow Node Functions.

One function per workflow node. Each node reads from WorkflowState,
calls the appropriate use-case/domain service, and returns a
partial WorkflowState update. Sequencing between nodes is owned by
the exported Archon workflow document (ADR-STR-033), never by Python.

Traceable to: FR-001, FR-012, FR-013, FR-019-v2, FR-077, ADR-STR-033, ADR-STR-003
...
from agentic_workflow.adapters.orchestration.state_mapper import StateMapper, WorkflowState
...
def should_continue_iterating(state: WorkflowState) -> str:
    """Routing predicate: decide whether to iterate or gate.
...
def node_agent_alpha_critique(state: WorkflowState) -> WorkflowState:
    ...
    try:
        from agentic_workflow.adapters.orchestration.nodes import _get_container
...
def node_step_6_trigger_impact(state: WorkflowState) -> WorkflowState:
    ...
    try:
        from agentic_workflow.adapters.orchestration.nodes import _get_container, node_impact_analysis
