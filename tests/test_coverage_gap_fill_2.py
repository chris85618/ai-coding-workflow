...
def _get_cfg(provider):
    mc = ModelConfig(provider=provider, model="test")
    return StrategyConfig(
        reasoning_model=mc, editing_model=mc, cheap_model=mc, default_model=mc, fallback_model=mc, enabled_providers=frozenset([provider])
    )

import sys
from unittest.mock import MagicMock

def test_llm_adapter_openai_init_success():
    cfg = _get_cfg("openai")
...
def test_llm_adapter_anthropic_init_success():
...
def test_sequential_adapter_success_call():
...
def test_sequential_adapter_is_connected_success():
    adapter = SequentialThinkingMCPAdapter("http://localhost:3000")
    with patch("urllib.request.urlopen") as mock_urlopen:
        mock_resp = MagicMock()
        mock_urlopen.return_value.__enter__.return_value = mock_resp
...
def test_file_repository_path_traversal(tmp_path):
    repo = FileTraceableIDRepository(str(tmp_path))
    with patch.object(pathlib.Path, "relative_to", side_effect=ValueError):
        with pytest.raises(ValueError, match="Path traversal detected"):
            repo._path_for("FR-001")

def test_file_repository_find_all_with_none(tmp_path):
    repo = FileTraceableIDRepository(str(tmp_path))
    ...
    # find_all will convert FOO_001.json -> stem FOO-001 -> _path_for(FOO-001) -> FOO-001.json which doesn't exist
    # so find_by_id returns None
    results = repo.find_all()
    assert len(results) == 0

def test_main_pragma():
