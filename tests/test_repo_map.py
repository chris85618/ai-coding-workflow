...
Traceable to: UC-012, INV-024, ALG-006
"""
...
@scenario("repo_map.feature", "Generate repo map within token budget")
def test_repo_map_within_budget():
    """SC-012: RepoMap generation respects token budget."""


@scenario("repo_map.feature", "PageRank prioritizes context files")
def test_pagerank_prioritizes_context():
    """SC-012: Context files rank higher in repo map."""


@scenario("repo_map.feature", "Empty project returns empty map")
...
def given_python_project(ctx, n, tmp_path):
...
def given_token_budget(ctx, budget):
    """Set the token budget."""
    ctx["budget"] = budget


@given("files A.py and B.py are in current context")
def given_context_files(ctx, tmp_path):
...
def given_empty_project(ctx, tmp_path):
...
def when_generate_repo_map(ctx):
...
def when_generate_with_personalization(ctx):
...
def then_token_count_ok(ctx, budget):
