...
class MicroValidationGraphBuilder(IMicroValidationGraphBuilder):
    ...
    def _setup_mv_nodes_rest(graph: StateGraph[WorkflowState, Any, Any]) -> None:
        """Add remaining nodes to validation graph."""
    ...
    @classmethod
    def _setup_mv_nodes(cls, graph: StateGraph[WorkflowState, Any, Any]) -> None:
        """Add first block of nodes to validation graph."""
        ...
        cls._setup_mv_nodes_rest(graph)
    ...
    def _setup_mv_edges_rest(cls, graph: StateGraph[WorkflowState, Any, Any]) -> None:
        """Add remaining edges to validation graph."""
        ...
        cls._setup_mv_edges_final(graph)

    @classmethod
    def _setup_mv_edges(cls, graph: StateGraph[WorkflowState, Any, Any]) -> None:
        """Add primary entry and first transitions to validation graph."""
        graph.set_entry_point("s0_format")
        graph.add_edge("s0_format", "s1_id")
        graph.add_edge("s1_id", "s2_fwd")
        graph.add_edge("s2_fwd", "s3_bwd")
        cls._setup_mv_edges_rest(graph)
    ...
    def build(cls) -> CompiledStateGraph[WorkflowState, Any, Any, Any]:
        ...
        cls._setup_mv_nodes(graph)
        cls._setup_mv_edges(graph)
